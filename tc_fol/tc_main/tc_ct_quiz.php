       ...
       <div class="row">
		...
		<button type="button" class="btn btn-success" data-toggle="modal" data-target="#bs-example-modal-sm">Create Quiz</button>
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
                      ...
                      <tbody>
					  		<?php 
								$query = mysqli_query($conn, "SELECT * FROM `ct_quiz` where id_for_teacher_quiz='".$_SESSION['tc_id']."';") or die(mysqli_error());
                        ...
                        <tr>
						 ...
						 <a href="tc_ct_quiz_view.php?quiz_id=<?php echo $id;?>&ahekdnsakmak8dasjkn34df" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> View </a>
                             
                            <a href="#edita<?php echo $id;?>" class="btn btn-primary btn-xs" data-toggle="modal"><i class="fa fa-pencil"></i> Edit </a>
                          
                            <a href="function/tc_fc_quiz/delete_quiz.php?idquiz=<?php echo $id;?>" onclick="return confirm('Do you want to delete?');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> Delete </a>
                          </td>
                        </tr>
								<?php } ?>
                      </tbody>
...
<!------===============adddd Quiz==========--------->
                 <div class="modal fade" id="bs-example-modal-sm" tabindex="-1" role="dialog" aria-hidden="true">
                    ...
                    <br>
                    <form class="form-horizontal form-label-left input_mask" action="function/tc_fc_quiz/add_quiz.php" method="POST">
...
<!------===============addddttttttooooooooooooo==========--------->
				   		...
				   		<?php 
								$query = mysqli_query($conn, "SELECT * FROM `ct_quiz`;") or die(mysqli_error());
								while($fetch = mysqli_fetch_array($query))
								{
										$id = $fetch['quiz_id'];
										$name = $fetch['name'];
                    ...
                    <div class="modal-dialog modal-sm">
                      <div class="modal-content">
                        ...
                        <div class="modal-header">
                          ...
                          <h4 class="modal-title" id="myModalLabel2">Add Quiz to:</h4>
                        </div>
                        <div class="modal-body">  
                    <br>
			 ...
			 <div class="col-md-12 col-sm-12 col-xs-12 form-group has-feedback">
					...
					<?php 
					$selectifexist = mysqli_query($conn, "select *from ct_addto where quizID='$id';") or die(mysqli_error()); 
					while($fors = mysqli_fetch_array($selectifexist))
					{ 
					  echo 'This quiz already give to section <b>'.$fors['sectionname'].'</b><br/>';  
					}
					?>
				  ...
				  <h3>Select Section</h3> 
					...
					?>
						...
						<br><br>
                          <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                          <button type="submit" name="btnaddto" class="btn btn-primary">Submit</button>
                        </div>
					</form> 
                      </div>
                    </div>
                  ...
                  <!-- /modals -->  
                    ...
                    <div class="modal-dialog modal-sm">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">  
                    <br>
                    <form class="form-horizontal form-label-left input_mask" action="function/tc_fc_quiz/edit_quiz.php" method="POST">
