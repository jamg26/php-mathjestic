                  ...
                  <div class="x_title">
                    ...
                    <div class="col-xs-3">
                      ...
                      <ul class="nav nav-tabs tabs-left">
                        <?php $count=0;
						$q_id=$_GET['activity_id'];
							$querys = mysqli_query($conn, "SELECT * FROM `ct_activity_questions`where activity_id='$q_id';") or die(mysqli_error());
						...
						<?php } ?>
						<br/>
						<form action="function/st_fc_activity/st_submit_answer.php?activity_id=<?php echo $q_id; ?>" method="POST">
						<input type="hidden" name="studentid" value="<?php echo $id; ?>"/>
						<button type="submit" name="btnsubmit" onclick="return confirm('Are you sure you want to submit?');" class="btn btn-primary">Submit</button>
                       </ul>
                    </div>
							...
							while($row = mysqli_fetch_array($querys))
							{
							 ...
							 else 
							 {
								 ...
								 <div class="tab-pane <?php echo $aa; ?>" id="aa<?php echo $question_id; ?>">
								  <p class="lead">Question Tab</p>
								  <div class="col-md-4">
								  <h2>Question: <?php echo $question; ?></h2>  <br/> 
								    <input type="text" name="<?php echo $question_id; ?>" class="form-control" placeholder="Enter here" required/>
